...
Person editPerson(Persons persons, int key) {
  ...
  Date birthDate = persons.list[key]!.birthDate;
  ...
  int citizenID = 0;
  if (citizen == "") citizenID = persons.list[key]!.citizenID;
  else citizenID = int.parse(citizen);

  print("NIT: (Atual: ${persons.list[key]!.taxID})");
  String tax = stdin.readLineSync()! + "";
  int taxID = 0;
  if (tax == "") taxID = persons.list[key]!.taxID;
  else taxID = int.parse(tax);
